...
class AutomatedFollowUpSystem:
    ...
    def __init__(self):
        # Use LLM router (required by rulepack - all LLM calls go through router)
        api_key = None
        if CONFIG_AVAILABLE and get_config:
            api_key = getattr(get_config(), 'openai_api_key', '') or os.getenv("OPENAI_API_KEY")
        else:
            api_key = os.getenv("OPENAI_API_KEY")
        
        self.router = LLMRouter(api_key=api_key) if (LLM_ROUTER_AVAILABLE and LLMRouter) else None
        self.email_handler = get_email_action_handler() if EMAIL_ACTION_HANDLERS_AVAILABLE else None
...
{user_name}""",
    ...
    # Removed _init_openai() - now using LLMRouter instead
                ...
                try:
                    ...
                    if result['success']:
                        try:
                            ...
                            logger.info(f"✅ Follow-up email personalized via LLM router")
                            return {
                                "subject": ai_content.get('subject', template.subject_template),
                                "body": ai_content.get('body', template.body_template)
                            }
                        except json.JSONDecodeError:
                            logger.warning("Failed to parse personalized content as JSON, using template")
                    else:
                        logger.warning(f"⚠️ LLM personalization failed: {result.get('error')}")
                    
                except Exception as e:
                    logger.warning(f"⚠️ LLM personalization failed: {e}")
