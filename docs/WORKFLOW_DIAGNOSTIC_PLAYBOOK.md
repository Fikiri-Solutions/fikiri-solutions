# Workflow Diagnostic — reusable checklist & assessment template

**Positioning:** You are a **system fixer first**. Automation is the *how*, after you’ve found **where time is wasted** and **where things break**.
...
| ☐ | Question | Capture answer |
|---|----------|----------------|
| ☐ | Median time to **first meaningful** response? (not auto-ack—unless that’s truly all they do) | Time: |
| ☐ | Best vs worst case in the last 30 days? | Range: |
| ☐ | Do they **measure** response time anywhere? | Y/N + where: |
| ☐ | After-hours / weekends—what’s promised vs what happens? | Actual behavior: |

**Gold:** repetition + delay + variance (great for quantifying loss).
...
| ☐ | Question | Capture answer |
|---|----------|----------------|
| ☐ | Follow-up **cadence** (attempt 2, 3—by when)? | Cadence: |
| ☐ | Who sends follow-ups? Template or one-off? | Process: |
| ☐ | Scheduling: tool, back-and-forth email, phone tag? | Scheduling: |
| ☐ | No-show / cancellation—what happens? | Policy + reality: |

### Section 7 — Reporting & truth
...
### Section 9 — Constraints (often missed—see Part D)

Capture **yes/no or short notes**:

| ☐ | Topic | Notes |
|---|-------|------|
| ☐ | Compliance / industry rules (HIPAA, FINRA, licensing, retention) | |
...
| ☐ | Task |
|---|------|
| ☐ | Read back **one** workflow definition in 2 sentences. |
| ☐ | List **top 3** repetition/delay/error findings. |
| ☐ | Parking lot: other workflows deferred. |
| ☐ | Next step: **Workflow Diagnostic** deliverables + fee (or disqualify clearly). |

---

## Part B — What you’re hunting for (quick scoring)

Use internally after the call—don’t turn it into a math exam for the client.
...
## Part C — Notion template (database + form)

### Option 1 — Database blueprint: `Diagnostics`

Use a **full-page database** in Notion (or inline on a “Sales / Diagnostics” page). Add properties in this order so **Notion Forms** map cleanly.

#### Build steps (~5 minutes)

1. `/database` → **New database** → name it **Diagnostics**.  
2. Add properties (**⋯** on table → **Properties** → **New property**) using the table below.  
3. Duplicate the default table view and configure **Views**.  
4. Optional: **Create form** from the database (see **Form mapping**).
...
| 15 | **Error risk score** | Number | Internal **1–5** (missed handoffs, wrong assignee). |
| 16 | **Variance note** | Text | Optional: e.g. “differs wildly by rep or day.” |
| 17 | **Compliance notes** | Text | HIPAA, TCPA/SMS, licensing, brand tone. |
| 18 | **Parking lot** | Text | Other workflows / ideas deferred. |
| 19 | **Next action** | Select | `Send Diagnostic SOW` · `Book Foundations` · `Nurture` · `No fit` · `Follow up — TBD` |
| 20 | **Fit call link** | URL | Calendly / booking. |
| 21 | **Recording link** | URL | If policy allows. |
| 22 | **Proposal link** | URL | Doc or PDF. |
| 23 | **Est. deal value** | Number | Optional internal $ for pipeline. |
| 24 | **Composite score** | Formula | Optional; see below. |

**Optional `Composite score` formula** (rename properties in the formula if yours differ):

`round((prop("Repetition score") + prop("Delay score") + prop("Error risk score")) / 3 * 10) / 10`

If any score can be empty, skip the formula and sort by **Delay score** or **Est. deal value** instead.

#### Views — configuration

| View name | Layout | Setup |
|-----------|--------|--------|
| **Pipeline** | Board | **Group by:** `Status`. **Sort:** `Session date` → ascending. **Card properties:** show `Industry`, `Next action` (Notion: card preview settings). |
| **This week** | Table | **Filter:** `Session date` → **is within** → `this week` (or `on or after` start of week **and** `on or before` end of week). Optional: `Status` **is not** `Won` / `Lost`. **Sort:** `Session date` ascending. |
| **Next 7 days** | Table | **Filter:** `Session date` **on or after** `@today` **and** **on or before** `@today + 7 days`. Use for session prep. |
| **By industry** | Gallery | **Group by:** `Industry`. Card: title + `Workflow one-liner` + `CRM health`. |
| **Hot leads** | Table | **Filter:** `Delay score` **≥** `4` **or** `Error risk score` **≥** `4` (adjust thresholds). **Sort:** `Est. deal value` descending. |
| **All** | Table | Master list; hide columns you rarely edit. |

#### Form mapping (Notion Form → this database)

- **On the public form:** Company/contact context, Industry, Workflow one-liner, Channels, Volume/week, First response time, Owner + backup, CRM tool, Top failure story, Compliance notes (optional).  
- **Internal only (after call):** Status, Session date, CRM health, three scores, Variance note, Parking lot, Next action, links, Est. deal value, Composite score.

Default new rows from the form to **Status** = `Lead` or `Scheduled` via a **database template** if you use one.

#### Database template (optional)

**⋯** → **Templates** → **New template**: pre-fill `Status` = `Scheduled`, empty scores—so every new diagnostic row starts consistent.

### Option 2 — Form (Notion form or Google Form) → creates row

**Block order (mirrors checklist)**
...
12. Upload screenshot (optional) — Google Drive link if not using file upload  

**After submit:** You still run a **live** diagnostic for nuance; the form reduces repeat questions and filters no-shows.
...
- **Owner vs office manager:** Who signs vs who runs the process? Misalignment kills adoption.  
- **“Champions” who don’t type:** Field staff live on phone/voice notes—email-first workflows fail unless you design for mobile + SMS.  
- **Tenured employee resistance:** New system can feel like judgment. Frame as “less scrambling,” not “monitoring.”
...
- **Phone-first demographics:** Older or local customers call; web forms undercount demand.  
- **Language:** Bilingual markets need templates + routing by language, not English-only copy-paste.  
- **Trust & tone:** Aggressive automation (too fast, too robotic) hurts referrals in relationship businesses.
...
- **Training burden:** If only you know how it works, it’s not done. Deliverables must include **who trains the next hire**.  
- **Maintenance owner:** Someone must own rule tweaks monthly—sell **care** or it rots.  
- **Realistic metrics:** First dashboard should be **3 KPIs they’ll actually open**, not a BI dissertation.

---

## Part E — Refine the process (lightweight, repeatable)

| Step | Refinement |
|------|------------|
| **Standardize** | Same checklist every time; compare scores across clients to learn verticals. |
| **Time-box** | 45–60 min live map + 15 min parking lot; async follow-up for numbers. |
| **Artifact-first** | End every diagnostic with: process map sketch + “top 3 leaks” + **one** quantified estimate (even a range). |
| **Disqualify kindly** | If no owner, no volume, or “not ready to change behavior,” say so—protects your margin and their trust. |
| **Debrief 15 min** | After each session: what surprised you? What would you ask next time? Update checklist once a quarter. |
| **Street test habit** | Before onsite/virtual: “If I were a customer, what breaks?”—feeds your first question. |
...
FIRST HOME: [inbox / CRM / sheet / head]
   ↓
OWNER: [name/role]   BACKUP: [gap?]
   ↓
RESPONSE: [median time]   VARIANCE: [best/worst]
   ↓
